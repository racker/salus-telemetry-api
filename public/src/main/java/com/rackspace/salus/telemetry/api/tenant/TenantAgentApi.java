...
@RestController
@RequestMapping("/api/{tenantId}/agents")
public class TenantAgentApi {

    private final AgentsCatalogService agentsCatalogService;

    @Autowired
    public TenantAgentApi(AgentsCatalogService agentsCatalogService
    ) {
    ...
    public CompletableFuture<AgentInstallSelector> install(@PathVariable String tenantId,
                                                           @RequestBody @Validated AgentInstallSelector agentInstallSelector) {
